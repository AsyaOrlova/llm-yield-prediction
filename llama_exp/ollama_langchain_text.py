...
model = OllamaLLM(model=mod, temperature=0.1, max_predict=50)
example_prompt = PromptTemplate.from_template("Reaction: {question}\nAnswer: {answer}")
instruction = zero_shot_template['text']
...
for k in tqdm([4]):
  for seed in [36, 42, 84, 200, 12345]:
    for idx in tqdm(df_test.index):
        ...
        print(out.split('</think>')[1])
        df_test.loc[idx, f'response_{k}_{seed}'] = out.split('</think>')[1]
    df_test.to_csv(f'{save_dataset_path}', index=False)
  df_test.to_csv(f'{save_dataset_path}', index=False)
